"""Data access layer for masterdata."""
...
from internal.db.models import Company as CompanyModel
from internal.db.models import Contact as ContactModel
from internal.db.models import Customer as CustomerModel
from internal.db.models import MasterDataVersion
from internal.db.models import Product as ProductModel


# Error codes (shared with main errors.py)
...
class MasterDataRepo:
    ...
    def get_version(self) -> int:
        ...
        try:
            ...
            # 如果查询失败，记录详细错误信息并回滚事务
            from observability.logging import get_logger
            logger = get_logger()
            logger.error(
                "Failed to get masterdata version",
                extra={
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "in_transaction": self.session.in_transaction() if hasattr(self.session, "in_transaction") else "unknown",
                },
                exc_info=True
            )
            # 回滚事务
            try:
                if hasattr(self.session, "in_transaction") and self.session.in_transaction():
                    self.session.rollback()
                    logger.info("Transaction rolled back after get_version failure")
